...
public class ImageUtils {
    ...
    private static String DEFAULT_DISK_CACHE_DIR = "image_disk_cache";
    ...
    private ImageUtils(File cacheDir) {
        mCacheDir = cacheDir;
    }
    ...
    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, ImageUtils.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    public ImageUtils launch() {
        ...
        else if (gear == ImageUtils.THIRD_GEAR)
            Observable.just(mFile)
                    .map(new Func1<File, File>() {
                        @Override
                        public File call(File file) {
    ...
    public ImageUtils load(File file) {
        mFile = file;
        return this;
    }

    public ImageUtils setCompressListener(OnCompressListener listener) {
        compressListener = listener;
        return this;
    }

    public ImageUtils putGear(int gear) {
        this.gear = gear;
        return this;
    }
...
//                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + ".jpg";
        String thumb = FileConfig.COMPRESS_IMG_PATH + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + ".jpg";
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) {
        ...
        while (stream.toByteArray().length / 1024 > 150 && options > 6) {//压缩至150k以下，size是计算出的合理值，如果对图片的大小没有特殊要求可将150改为size
            stream.reset();
            options -= 6;
            bitmap.compress(Bitmap.CompressFormat.JPEG, options, stream);
        }
